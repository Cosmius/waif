...
fn check_reports_every_bad_artifact_before_exiting_one() {
    ...
    assert!(stderr.contains("goal.md:1:"));
    assert!(stderr.contains("plan.md:1:"));
...
fn check_without_path_uses_the_current_task() {
    ...
    let valid = "# Example\n- Status: accepted\n## Details\nProse.\n";
    fs::write(task.join("goal.md"), valid).expect("goal should be written");
    fs::write(step.join("review1.md"), valid).expect("review should be written");
    ...
    assert!(output.status.success());
    assert!(stdout.contains("goal.md: valid"));
    assert!(stdout.contains("review1.md: valid"));
    assert!(stdout.contains("checked 2 artifact(s); 0 invalid"));
}
