...
use crate::artifact::{Artifact, Item, ItemForm, Section};
...
fn validate_items(
    ...
    for item in section.items() {
        ...
        let Some(number) = valid_item_number(identifier_text, rule.prefix) else {
            diagnostics.push(Diagnostic::error(
        ...
        let duplicate = !seen.insert((rule.prefix, number));
        if duplicate {
            diagnostics.push(Diagnostic::error(
                identifier.span().start_line(),
                format!(
        ...
        if !duplicate {
            if let Some(previous) = last_numbers.get(rule.prefix) {
                if number < *previous {
                    diagnostics.push(Diagnostic::error(
                        identifier.span().start_line(),
                        format!(
        ...
        {
            last_numbers.insert(rule.prefix, number);
        }
    }
}
